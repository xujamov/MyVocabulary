package com.example.myvocabulary2
...
class MainActivity : AppCompatActivity() {
    ...
    fun saveRecord(view: View){
        ...
        if(id.trim()!="" && name.trim()!=""){
            val status = databaseHandler.addEmployee(EmpModelClass(Integer.parseInt(id),name))
            if(status > -1){
    ...
    fun updateRecord(view: View){
        ...
        dialogBuilder.setPositiveButton("Update", DialogInterface.OnClickListener { _, _ ->
            ...
            if(updateId.trim()!="" && updateName.trim()!=""){
                ...
                val status = databaseHandler.updateEmployee(EmpModelClass(Integer.parseInt(updateId),updateName))
                if(status > -1){
